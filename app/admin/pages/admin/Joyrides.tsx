...
const Joyrides = () => {
  ...
  const [joyrides, setJoyrides] = useState<Joyride[]>([]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const capacityNum = parseInt(String(formData.capacity || '0'), 10);
      if (!formData.name || !formData.aircraft || !capacityNum || !formData.price) {
        throw new Error('Please fill Name, Aircraft, Capacity, and Price');
      }
      const payload = {
        name: formData.name,
        model: formData.aircraft || 'Taxi',
        capacity: capacityNum,
        range: formData.duration || 'N/A',
        speed: 'N/A',
        pricePerHour: Number(String(formData.price).replace(/[^0-9.]/g, '')) || 0,
        description: formData.description,
        image: formData.imageUrl,
        status: formData.available ? 'available' : 'maintenance',
        isActive: formData.bookingsEnabled,
      };
      ...
      setJoyrides(prev => [
        {
          ...
          aircraft: created.model,
          duration: formData.duration,
          capacity: created.capacity,
          price: payload.pricePerHour,
          location: formData.location,
          imageUrl: created.image,
          type: formData.type,
          available: created.status === 'available',
          bookingsEnabled: !!created.isActive,
        },
        ...prev,
      ]);
      ...
      toast({ title: 'Added', description: 'Taxi record saved to database.' });
    } catch (err: any) {
      toast({ title: 'Create failed', description: err?.message || 'Unable to add taxi item', variant: 'destructive' as any });
    }
    return; // end POST branch

    // legacy local-only add (kept for reference, not used)
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const mapped: Joyride[] = (Array.isArray(data) ? data : []).map((d: any) => ({
          id: d._id,
          name: d.name,
          description: d.description || '',
          aircraft: d.model || 'Taxi',
          duration: typeof d.range === 'string' ? d.range : '',
          capacity: d.capacity ?? 0,
          price: d.pricePerHour ?? 0,
          location: '',
          imageUrl: d.image || '',
          type: 'helicopter',
          available: (d.status || 'available') === 'available',
          bookingsEnabled: !!d.isActive,
        }));
        setJoyrides(mapped);
      } catch (e: any) {
        toast({ title: 'Load failed', description: e?.message || 'Unable to load taxi items', variant: 'destructive' as any });
      }
    })();
  }, []);
