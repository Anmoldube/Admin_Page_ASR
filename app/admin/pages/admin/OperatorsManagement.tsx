...
const OperatorsManagement = () => {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        // No operators API yet: try debug endpoint in dev, else empty
        const res = await fetch('/api/debug/users');
        if (res.ok) {
          const data = await res.json();
          const items = Array.isArray(data) ? data : (data.items || []);
          const mapped: Operator[] = items.map((u:any, i:number) => ({
            id: u._id || String(i),
            companyName: u.company || 'Operator',
            contactPerson: u.name || u.email || 'Contact',
            email: u.email || '',
            phone: u.phone || '',
            rating: 5,
            totalFlights: 0,
          ...
          if (isMounted) setOperators(mapped);
        }
      } catch (e:any) {
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        {loading && <div className="text-sm text-muted-foreground">Loading operators…</div>}
