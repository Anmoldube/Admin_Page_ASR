...
const EmptyLegs = () => {
  ...
  const [deals, setDeals] = useState<EmptyLeg[]>([]);
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const filtered = items.filter((d) => Array.isArray(d.tags) ? d.tags.includes('empty-leg') : false);
        const mapped: EmptyLeg[] = filtered.map((d) => ({
          id: d._id,
          route: `${d.from} → ${d.to}`,
          aircraft: d.aircraft,
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        {loading && <div className="text-sm text-muted-foreground">Loading empty legs…</div>}
