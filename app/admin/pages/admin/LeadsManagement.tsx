  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const mapped: Lead[] = (Array.isArray(items) ? items : []).map((b:any) => ({
          id: b._id,
          name: b.user?.name || 'Unknown',
          email: b.user?.email || '',
          phone: '',
          route: `${b.flight?.departure?.airport || ''} → ${b.flight?.arrival?.airport || ''}`,
          date: b.flight?.departure?.time ? new Date(b.flight.departure.time).toISOString().slice(0,10) : '',
          status: (b.status as any) || 'New',
          source: 'Website',
          priority: 'Medium',
          notes: '',
          assignedTo: 'Unassigned',
        }));
        if (isMounted) setLeads(mapped);
      } catch (e:any) {
